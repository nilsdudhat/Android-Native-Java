        ...
        Glide.with(MyApplication.getInstance())
                ...
                .into(new SimpleTarget<Bitmap>() {
                    ...
                    public void onLoadFailed(Exception e, Drawable errorDrawable) {
                        ...
                        e.printStackTrace();
                        Toast.makeText(MyApplication.getInstance(), e.toString(), Toast.LENGTH_LONG).show();
                    }
                    ...
                    public void onResourceReady(final Bitmap resource, GlideAnimation<? super Bitmap> glideAnimation) {
                        new AsyncTask<Void, Void, Void>() {
                            ...
                            protected Void doInBackground(Void... params) {
                                File dir = new File(MyApplication.getInstance().getFilesDir(), FOLDER_NAME);
                                if (!dir.exists()) {
                                ...
                                if (succesful) {
                                    ...
                                    ArtistSignatureUtil.getInstance(MyApplication.getInstance()).updateArtistSignature(artist.getName());
                                    MyApplication.getInstance().getContentResolver().notifyChange(MediaStore.Audio.Artists.EXTERNAL_CONTENT_URI, null); // trigger media store changed to force artist image reload
                                }
                                return null;
                            }
                        }.execute();

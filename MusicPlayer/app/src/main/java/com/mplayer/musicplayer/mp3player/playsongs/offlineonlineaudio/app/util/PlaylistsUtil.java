...
import com.mplayer.musicplayer.mp3player.playsongs.offlineonlineaudio.app.helper.M3UWriter;
...
public class PlaylistsUtil {
    ...
    public static void deletePlaylists(@NonNull final Context context, @NonNull final List<Playlist> playlists) {
        ...
        for (int i = 0; i < playlists.size(); i++) {
            ...
            String playlistid = getPlayListId(context, playlists.get(i));
...
//        Toast toast = Toast.makeText(context,selectedplaylist + " Deleted", Toast.LENGTH_SHORT);
//        toast.show();
...
//        } catch (SecurityException ignored) {
//            Log.d("--delete--", "deletePlaylists: " + ignored.getMessage());
//        }
    }

    public static String getPlayListId(Context context, Playlist playlist) {
        ...
        String[] whereVal = {String.valueOf(playlist)};
        String[] projection = {playlistid, playlistname};
        ContentResolver resolver = context.getContentResolver();

        Cursor record = resolver.query(MediaStore.Audio.Playlists.EXTERNAL_CONTENT_URI, projection, where, whereVal, null);

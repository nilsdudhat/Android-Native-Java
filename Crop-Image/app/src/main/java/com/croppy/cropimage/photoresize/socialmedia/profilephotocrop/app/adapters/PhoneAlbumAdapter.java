...
import com.croppy.cropimage.photoresize.socialmedia.profilephotocrop.app.ui.activities.AlbumListActivity;
...
import com.croppy.cropimage.photoresize.socialmedia.profilephotocrop.app.views.SquareLayout;
...
public class PhoneAlbumAdapter extends RecyclerView.Adapter<PhoneAlbumAdapter.ViewHolder> {
    ...
    public ArrayList<PhoneAlbum> al_album = new ArrayList<>();
    Activity baseActivity;

    public PhoneAlbumAdapter(Activity baseActivity, ArrayList<PhoneAlbum> albumArrayList) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int i) {
        ...
        Glide.with(baseActivity).load(((PhoneAlbum) this.al_album.get(position)).getCoverUri()).listener(new RequestListener<Drawable>() {
        ...
        ((ViewHolder) holder).tv_album_name.setText(((PhoneAlbum) this.al_album.get(position)).getName());

        ((ViewHolder) holder).itemView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Share.lst_album_image.clear();
                for (int i = 0; i < ((PhoneAlbum) al_album.get(position)).getAlbumPhotos().size(); i++) {
                    Share.lst_album_image.add(((PhonePhoto) ((PhoneAlbum) al_album.get(position)).getAlbumPhotos().get(i)).getPhotoUri());
                }
